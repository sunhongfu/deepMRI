...
%
% Optional Inputs are:
% 2. mask: Brain Mask ROI, whose size is the same as the lfs input (1-st
%    echo); default: ones;
% 3. voxel_size: image resolution; default: [1 1 1] mm isotropic;
% 4. output_dir: directory/folder for output of temporary and final results
%    default: pwd (current working directory)
% 5. save_flag (bool): flag for saving the temporary output files.
...
[xQSM_dir, ~, ~] = fileparts(which('xQSM.m'));
...
[mask, vox, output_dir] = parse_iQSM_inputs(size(lfs), varargin{:});
...
save_nii(nii, [output_dir,'/xQSM.nii']);

cprintf('*[0, 0, 0]', 'xQSM results successfully returned! \n');


    function [mask, vox, output_dir] = parse_iQSM_inputs(imsize, varargin)
