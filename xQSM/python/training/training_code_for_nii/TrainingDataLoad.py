...
class DataSet(data.Dataset):
    def __init__(self, root, list_path, transform=None):
        ...
        for name in self.img_ids:
            lfs_file = self.root + ("/Field_VIVO/%s-Field_NIFTI.nii" % name)
            chi_file = self.root + ("/QSM_VIVO/%s-Phantom_NIFTI.nii" % name)
            self.files.append({
                #"img": img_file,
                "lfs": lfs_file,
                "chi": chi_file,
    ...
    def __getitem__(self, index):
        ...
        name = datafiles["name"]
        ## nifti read codes. 
        nibimage = nib.load(datafiles["lfs"])
        niblabel = nib.load(datafiles["chi"])

        lfs = nibimage.get_data() 
        chi = niblabel.get_data() 
